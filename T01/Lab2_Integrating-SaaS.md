# Lab 2: Integrating SaaS Services Available on the Azure Platform

# Objectivies
After you complete this lab, you'll be able to:
- create a function app that uses Azure Cognitive Services
- create a logic app that leverages the function app created in this lab
...
1. In the **Choose File to Upload** dialog box, navigate to the **\T01\files**  folder, select the **cognitive-template.json** file, and click **Open**. This will load the following content into the template editor pane:
...
1. Back on the **Custom deployment** blade, perform the following tasks:
    ...
    - In the **Resource group** section, ensure that your resource group is selected.
...
2. Wait for the deployment to complete before you proceed to the next step.

3. In the hub menu of the Azure portal, click **Resource groups**.

4. On the **Resource groups** blade, click your resource group.

5. On the resource group blade, locate the **Deployments** header at the top of the blade and click the below the **Deployments** label, which indicates the number of successful deployments.

6. On the deployments blade, click the name of the most recent deployment.

7. On the **Microsoft.Template - Overview** blade, click **Outputs**.

8. On the **Microsoft.Template - Outputs** blade, identify the values of **cognitiveEndpointUrl** and **cognitiveEndpointKey** outputs. Record these values, since you will need them later in the lab.
...
1. Navigate to the logic app you created in the previous task.
...
1. Replace the default JSON template with the following template that includes an HTTP trigger (**\T01\files\\logic-app.json**) and save your changes:
...
1. In the **You have no storage mounted** pane, click **Show advanced settings**, perform the following tasks:
    ...
    - In the **Resource group** section, select the **Use existing** option and then, select your resource group.
